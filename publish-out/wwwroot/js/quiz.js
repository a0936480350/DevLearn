...
async function openQuiz() {
    ...
    document.getElementById('quizContent').innerHTML = `<div class="quiz-loading">⏳ ${__t('ch-quiz-loading')}</div>`;

    // 改用新的 ChapterQuiz API（回傳 JSON）
    const res = await fetch(`/Quiz/ChapterQuiz?chapterId=${CHAPTER_ID}&count=5`);
    const data = await res.json();
    if (!data.questions || data.questions.length === 0) {
        document.getElementById('quizContent').innerHTML = `<div class="quiz-loading">${__t('quiz-load-failed')}</div>`;
        return;
    }
...
function renderQuiz() {
    ...
    container.innerHTML = `
        <div id="quiz-questions">
            ${quizQuestions.map((q, i) => `
                <div class="quiz-question">
                    ...
                    <div class="q-options">
                        ${q.type === 'fillin' ? `
                            <input type="text" name="q_${q.id}" class="quiz-input" placeholder="${__t('quiz-fillin-ph')}" />
                        ` : (q.options || []).map(opt => `
        ...
        <div class="quiz-actions">
            <button class="btn-retry" onclick="closeQuiz()">${__t('btn-cancel')}</button>
            <button class="btn-submit" onclick="submitQuiz()">📝 ${__t('quiz-submit-btn')}</button>
        </div>`;
}
...
function renderResult(data) {
    ...
    const msg = passed
        ? (pct >= 80 ? `🎉 ${__t('quiz-grade-excellent')}！` : `👍 ${__t('quiz-grade-pass')}！`)
        : `📚 ${__t('quiz-grade-fail')}（需 60% 以上）`;
    ...
    document.getElementById('quizContent').innerHTML = `
        ...
        ${data.details.map((r, i) => `
            <div class="q-result">
                ...
                <div style="font-size:.82rem;margin-bottom:.3rem;">
                    ${__t('quiz-your-answer')}<strong style="color:${r.isCorrect ? '#7EE787' : '#F78166'}">${r.yourAnswer || __t('quiz-no-answer')}</strong>
                    ${!r.isCorrect ? `&nbsp;&nbsp;${__t('quiz-correct-answer')}<strong style="color:#7EE787">${r.correctAnswer}</strong>` : ''}
                </div>
        ...
        <div class="quiz-actions">
            ...
            ${passed ? `<button class="btn-submit" onclick="location.reload()">✅ ${__t('ch-complete')}</button>` : `<button class="btn-submit" onclick="closeQuiz()">${__t('quiz-close-btn')}</button>`}
        </div>`;
}
